...
class Twitter(data.Dataset):
    ...
    def __init__(self, path, text_field, label_field, **kwargs):
        ...
        Arguments:
            ...
            text_field: The field that will be used for text data1.
            label_field: The field that will be used for label data1.
        ...
        tweets = open(os.getcwd()+"/data1/"+config["train_file"], "r+")
    ...
    def splits(cls, text_field, label_field, root='',
        ...
        Arguments:
            ...
            label_field: The field that will be used for label data1.
            root: Root dataset storage directory. Default is '.data1'.
    ...
    @classmethod
    def iters(cls, batch_size=1024, device=0, root='.data1', vectors=None, **kwargs):
...
def main(argv):
    ...
    output = open(os.getcwd() + "/bert/data1/proc/train_proc_"+ str(config["number_tweets"]) +".json", "w+")
    neg = open(os.getcwd() + "/src/data1/train_neg_full.txt", "r+")
    pos = open(os.getcwd() + "/src/data1/train_pos_full.txt", "r+")
...
def main_test_proc(argv):
    test = open(os.getcwd() + "/src/data1/test_data.txt", "r+")
    output = open(os.getcwd() + "/bert/data1/test_preprocessed.txt", "w+")
    processor = TweetProcessor()
...
def main_test(argv):
    test = open(os.getcwd() + "/src/data1/test_data.txt", "r+")
    output = open(os.getcwd() + "/bert/data1/test_data.txt", "w+")
    for i, line in enumerate(test.readlines()):
        line = line.split(",")[1]
        tweet = line.replace("\n", "")
        output.write(tweet+"\n")
...
def main_test_xlnet(argv):
    test = open(os.getcwd() + "/src/data1/test_data.txt", "r+")
    output = open(os.getcwd() + "/bert/data1/test_data_xlnet.txt", "w+")
    processor = TweetProcessor()
    for i, line in enumerate(test.readlines()):
        line = line.split(",")[1]
        tweet = processor.process(line.replace("\n", ""))
...
def main_xlnet(argv):
    # neg / pos tweets in separate files
    output = open(os.getcwd() + "/bert/data1/proc/train_proc_pos_full.txt", "w+")
    file = open(os.getcwd() + "/src/data1/train_pos_full.txt", "r+")
    processor = TweetProcessor()
    last_tweet = ""
    for i, line in enumerate(file.readlines()):
        # tweet = line.replace("\n", "")
